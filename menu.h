...
private:
    ...
    bool file_menu() {
        ...
        while (!exit) {
            selected_index = menu_box({
                "Создать базу данных",
                "Выбрать базу данных",
                "Выбрать зашифрованную базу данных",
                "Выход"
                });
            if (selected_index == 0) {
                name = prompt_box({
                    "Введите название новой БД",
                    "(только цифры и английские буквы)"
                    });
                ...
                name = prompt_box({
                    "Введите название существующей БД",
                    "(только цифры и английские буквы)"
                    });
                check = check_database_name(name);
                if (check) {
                    ...
                    else alert_box({ "БД не существует или существует в зашифрованном виде!" });
                }
                else {
                    alert_box({ "Неправильное имя БД!" });
                }
                ...
                if (check) {
                    ...
                    if (check) {
                        ...
                        check = decrypt_file(std::string(name + ".bin.enc").c_str(), std::string(name + ".bin").c_str(), passwd.c_str());
                        if (check) {
                            alert_box({ "БД выбрана!" });
                            set_path(name + ".bin");
                            break;
                        }
                        else alert_box({ "Вы ввели неправильный пароль!" });
                    }
                    else alert_box({ "БД не существует или существует в незашифрованном виде!" });
                }
                else alert_box({ "Неправильное имя БД!" });
            }
    ...
    void change_exams(Student& record, short session_index) {
        ...
        while (!exit) {
            ...
            for (char i = 0; i < 10; i++) {
                lines.push_back("Предмет [" + record.get_subject_name(session_index, i) + "] Оценка [" + record.get_subject_mark(session_index, i) + "]");
            }
            selected_index = menu_box(lines);
            if (selected_index == 0) {
                exit = true;
                ...
                selected_index = menu_box({ "Выход", "Изменить название предмета", "Изменить оценку за предмет" });
                if (selected_index == 1) {
                    tmp_str = prompt_box({ "Введите название предмета:" });
                    if (record.set_subject_name(tmp_str, session_index, subject_index)) alert_box({ "Вы неправильно ввели название предмета!" });
                    else alert_box({ "Название предмета изменено." });
                }
                else if (selected_index == 2) {
                    selected_index = menu_box({ "Незачет", "Зачет", "Отлично", "Хорошо", "Удовлетворительно", "Неудовлетворительно" });
    ...
    bool change_student_record(Student& record) {
        ...
        while (!exit) {
            ...
            if (selected_index == 0) {
                ...
                if (copied_record.set_surname(tmp_str)) alert_box({ "Вы неправильно ввели фамилию!" });
                else alert_box({ "Фамилия изменена." });
            }
            else if (selected_index == 4) {
                tmp_str = prompt_box({ "Введите имя:" });
                if (copied_record.set_name(tmp_str)) alert_box({ "Вы неправильно ввели имя!" });
                else alert_box({ "Имя изменено." });
            }
            else if (selected_index == 5) {
                tmp_str = prompt_box({ "Введите отчество:" });
                if (copied_record.set_patromymic(tmp_str)) alert_box({ "Вы неправильно ввели отчество!" });
                else alert_box({ "Отчество изменено." });
            }
            else if (selected_index == 6) {
                tmp_str = prompt_box({ "Введите название факультета:" });
                if (copied_record.set_faculty(tmp_str)) alert_box({ "Вы неправильно ввели название факультета!" });
                else alert_box({ "Название факультета изменено." });
            }
            else if (selected_index == 7) {
                tmp_str = prompt_box({ "Введите название кафедры:" });
                if (copied_record.set_department(tmp_str)) alert_box({ "Вы неправильно название кафедры!" });
                else alert_box({ "Название кафедры изменено." });
            }
            else if (selected_index == 8) {
                tmp_str = prompt_box({ "Введите название группы:" });
                if (copied_record.set_group(tmp_str)) alert_box({ "Вы неправильно ввели название группы!" });
                else alert_box({ "Название группы изменено." });
            }
            else if (selected_index == 9) {
                tmp_str = prompt_box({ "Введите номер зачетной книжки:" });
                if (copied_record.set_record_book_number(tmp_str, record_book_number_in_database(copied_record.get_record_book_number()))) alert_box({ "Вы неправильно ввели номер зачетной книжки!" });
                else alert_box({ "Номер зачетной книжки изменен." });
            }
            else if (selected_index == 10) {
                tmp_str = prompt_box({ "Введите год поступления:" });
                if (copied_record.set_start_year(tmp_str)) alert_box({ "Вы неправильно ввели год поступления!" });
                else alert_box({ "Год поступления изменен." });
            }
            else if (selected_index == 11) {
                tmp_str = prompt_box({ "Введите дату рождения:" });
                if (copied_record.set_date_of_birth(tmp_str)) alert_box({ "Вы неправильно ввели дату рождения!" });
                else alert_box({ "Дата рождения изменена." });
            }
    ...
    void list_of_student_records() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            lines.push_back(record.get_surname() + " " + record.get_name()[0] + "." + record.get_patronymic()[0] + ". " + record.get_record_book_number() + " " + record.get_start_year());
        }
        bool exit = false;
        ...
        while (!exit) {
            ...
            if (selected_index == 0) {
                ...
                if (selected_index == 1) {
                    ...
                    if (!change_student_record(record))
                        set_record(record, save_index);
                }
                else if (selected_index == 2) {
    ...
    void option_25() {
        ...
        if (records.empty()) {
            ...
            for (int i = 0; i < records.size(); i++) {
                lines.push_back(records[i].get_surname() + " " + records[i].get_name()[0] + "." + records[i].get_patronymic()[0] + ". " + records[i].get_record_book_number() + " " + records[i].get_start_year());
            }
            alert_box(lines);
        }
    }
    ...
    void student_menu() {
        ...
        while (!exit) {
            selected_index = menu_box({
                ...
                "Список студентов (удалить или изменить записи)",
                "Выполнить вариант 25",
                "Выход с шифрованием файла",
                "Выход"
                });
